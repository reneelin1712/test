...
def evaluate_log_prob(test_traj,test_weather,model):
    log_prob_list = []
    ...
    for episode, weather in zip(test_traj, test_weather):
        ...
        for x in episode:
            with torch.no_grad():
                next_prob = torch.log(model.get_action_prob(torch.LongTensor([x.cur_state]).to(device), des, weather_var)).squeeze()
...
def evaluate_model(target_od, target_traj, target_weather, model, env, n_link=423):
    ...
    for ori, des, weather in zip(target_o[1:], target_d[1:], target_w[1:]):
        if des == curr_des and weather == curr_weather:
        # if des == curr_des:
            if ori == curr_ori:
                learner_traj.append(sample_path)
                continue
            ...
            curr_ori, curr_des, curr_weather = ori, des, weather
            des_ts = (torch.ones_like(state_ts) * curr_des).to(device)
            weather_ts = (torch.ones_like(state_ts) * curr_weather).to(device)
            # action_prob = model.get_action_prob(state_ts, des_ts).detach().cpu().numpy()  # 714, 8
            action_prob = model.get_action_prob(state_ts, des_ts, weather_ts).detach().cpu().numpy()  # 714, 8
