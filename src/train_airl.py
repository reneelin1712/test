...
def update_params_airl(batch, i_iter):
    ...
    weathers = torch.from_numpy(np.stack(batch.weather)).long().to(device)

    with torch.no_grad():
        values = value_net(states, destinations,weathers)
        next_values = value_net(next_states, destinations, weathers)
        fixed_log_probs = policy_net.get_log_prob(states, destinations, weathers, actions)
    ...
    for _ in range(epoch_disc):
        ...
        s_expert_weather = expert_weather[indices].to(device)
        
        with torch.no_grad():
            expert_log_probs = policy_net.get_log_prob(s_expert_st, s_expert_des, s_expert_weather, s_expert_ac)
        # states, des, log_pis, next_states
        g_o = discrim_net(states, destinations, actions, fixed_log_probs, next_states, weathers)
        e_o = discrim_net(s_expert_st, s_expert_des, s_expert_ac, expert_log_probs, s_expert_next_st, s_expert_weather)
    ...
    rewards = discrim_net.calculate_reward(states, destinations, actions, fixed_log_probs, next_states, weathers).squeeze()
    ...
    for _ in range(optim_epochs):
        ...
        states, destinations, actions, returns, advantages, fixed_log_probs, weathers= \
            states[perm].clone(), destinations[perm].clone(), actions[perm].clone(), returns[perm].clone(), advantages[
                perm].clone(), fixed_log_probs[perm].clone(), weathers[perm].clone()
        for i in range(optim_iter_num):
            ind = slice(i * optim_batch_size, min((i + 1) * optim_batch_size, states.shape[0]))
            states_b, destinations_b, actions_b, advantages_b, returns_b, fixed_log_probs_b, weathers_b = \
                states[ind], destinations[ind], actions[ind], advantages[ind], returns[ind], fixed_log_probs[ind], weathers[ind]
            batch_value_loss, batch_policy_loss = ppo_step(policy_net, value_net, optimizer_policy, optimizer_value, 1,
                                                           states_b, destinations_b, weathers_b, actions_b, returns_b,
...
if __name__ == '__main__':
    ...
    expert_st, expert_des, expert_weather, expert_ac, expert_next_st = env.import_demonstrations(train_p)
    to_device(device, expert_st,expert_weather, expert_des, expert_ac, expert_next_st)
