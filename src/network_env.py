...
Step = namedtuple('Step', ['cur_state', 'action', 'next_state', 'reward', 'done', 'weather'])
...
class RoadWorld(object):
    ...
    def reset(self, st=None, des=None):
        ...
        self.cur_weather = np.random.choice([0, 1])  # 0: sunny, 1: rainy

        return self.cur_state, self.cur_des, self.cur_weather
    ...
    def import_demonstrations(self, demopath, od=None, n_rows=None):
        ...
        expert_st, expert_des, expert_ac, expert_st_next, expert_weather  = [], [], [], [], []
        for demo_str, demo_des, demo_weather in zip(demo['path'].tolist(), demo['des'].tolist(), demo['weather'].tolist()):
            ...
            for i0 in range(1, len_demo):
                ...
                expert_weather.append(demo_weather)
        return torch.LongTensor(expert_st), torch.LongTensor(expert_des), torch.LongTensor(expert_weather), torch.LongTensor(expert_ac), torch.LongTensor(
            expert_st_next)
    ...
    def import_demonstrations_step(self, demopath, n_rows=None):
        ...
        for demo_str, demo_des, demo_weather in zip(demo['path'].tolist(), demo['des'].tolist(), demo['weather'].tolist()):
            ...
            for i0 in range(1, len_demo):
                ...
                episode.append(Step(cur_state=cur_state, action=action, next_state=next_state, reward=reward, done=is_done, weather=demo_weather))
