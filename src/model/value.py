...
class ValueCNN(nn.Module):
    ...
    def process_features(self, state, des, weather):
        ...
        weather_feature = weather.unsqueeze(-1).float()

        feature = torch.cat([path_feature, edge_feature, weather_feature], -1)  # [batch_size, n_path_feature + n_edge_feature]
        return feature

    def forward(self, state, des, weather):  # 这是policy
