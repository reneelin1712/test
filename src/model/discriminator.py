...
class DiscriminatorAIRLCNN(nn.Module):
    ...
    def process_neigh_features(self, state, des, weather):
        ...
        neigh_weather_feature = weather.unsqueeze(-1).unsqueeze(-1).repeat(1, neigh_path_feature.size(1), 1)
        neigh_feature = torch.cat([neigh_weather_feature, neigh_path_feature, neigh_edge_feature, neigh_mask_feature],
                                  -1)
    ...
    def process_state_features(self, state, des, weather):
        path_feature = self.path_feature[state, des, :]  # 实在不行你也可以把第一个dimension拉平然后reshape 一下
        edge_feature = self.link_feature[state, :]

        weather_feature = weather.unsqueeze(-1)
        feature = torch.cat([weather_feature, path_feature, edge_feature], -1)  # [batch_size, n_path_feature + n_edge_feature]
        return feature

    def f(self, state, des, act, next_state, weather):
        """rs"""
        x = self.process_neigh_features(state, des, weather)
        ...
        """hs"""
        x_state = self.process_state_features(state, des, weather)
        x_state = F.leaky_relu(self.h_fc1(x_state), 0.2)
        x_state = F.leaky_relu(self.h_fc2(x_state), 0.2)
        x_state = self.h_fc3(x_state)

        """hs_next"""
        next_x_state = self.process_state_features(next_state, des, weather)
    ...
    def forward(self, states, des, act, log_pis, next_states, weathers):
        # Discriminator's output is sigmoid(f - log_pi).
        return self.f(states, des, act, next_states, weathers) - log_pis

    def calculate_reward(self, states, des, act, log_pis, next_states, weathers):
        with torch.no_grad():
            logits = self.forward(states, des, act, log_pis, next_states, weathers)
