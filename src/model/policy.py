...
class PolicyCNN(nn.Module):
    ...
    def process_features(self, state, des, weather):
        ...
        weather_feature = weather.unsqueeze(-1).unsqueeze(-1).repeat(1, neigh_path_feature.size(1), 1)
    ...
    def get_action_prob(self, state, des, weather):
        x = self.process_features(state, des, weather)
        x = self.forward(x)
        x_mask = self.policy_mask[state]  # [batch, 8]
        x = x.masked_fill((1 - x_mask).bool(), -1e32)
        return F.softmax(x, dim=1)

    def get_action_log_prob(self, state, des, weather):
        x = self.process_features(state, des, weather)
        x = self.forward(x)
        x_mask = self.policy_mask[state]  # [batch, 8]
        x = x.masked_fill((1 - x_mask).bool(), -1e32)
    ...
    def select_action(self, state, des, weather):
        action_prob = self.get_action_prob(state, des, weather)
        action = torch.distributions.Categorical(action_prob).sample()
        return action

    def get_kl(self, state, des, weather):
        action_prob1 = self.get_action_prob(state, des, weather)
        action_prob0 = action_prob1.detach()
        kl = action_prob0 * (torch.log(action_prob0) - torch.log(action_prob1))
        return kl.sum(1, keepdim=True)

    def get_log_prob(self, state, des, weather, actions):
        action_prob = self.get_action_prob(state, des, weather)
        return torch.log(action_prob.gather(1, actions.long().unsqueeze(1)))

    def get_fim(self, state, des, weather):
        action_prob = self.get_action_prob(state, des, weather)
        M = action_prob.pow(-1).view(-1).detach()
        return M, action_prob, {}
    

class PolicyCNNWrapper(nn.Module):
    def __init__(self, policy_net):
        super(PolicyCNNWrapper, self).__init__()
        self.policy_net = policy_net

    def forward(self, input_tuple):
        state, des, weather = input_tuple
        x = self.policy_net.process_features(state, des, weather)
